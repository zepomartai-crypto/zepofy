...
const WhatsAppIntegration = ({ successMessage, errorMessage, setSuccessMessage, setErrorMessage }) => {
    ...
    const handleFacebookConnect = () => {
        if (!window.FB) {
            setErrorMessage('Facebook SDK is still loading. Please wait 2 seconds and try again.');
            return;
        }
                ...
                if (response.authResponse && response.authResponse.code) {
                    ...
                    if (sessionDataRef.current) {
                        ...
                        setTimeout(() => {
                            ...
                            if (!sessionDataRef.current) {
                                setFbLoading(false);
                                setErrorMessage('Facebook login completed, but WABA session info was not received. Please try again.');
                            }
                        }, 20000);
                    }
                } else {
